...
const Modal = (props) => {
    ...
    //console.log(data)
    //var kakakaka = data.hl7;
    //var message = hl7parser.create(kakakaka);
    //var message = hl7parser.create(""+data.hl7.toString());
    ...
    //console.log(data.hl7)
    //console.log(typeof data.hl7)
    var hl7 = "MSH|^~.&|||||20221113034406||EHC^E01^EHC_E01|123|P|2.6||||||||||||||\rIVC|1|||OR|NORM|FN|20221113034406|202970&KRW||레메디피부과의원^^^^^^^^^2221166666|SJlife||||||||^박^의사||||||AMB||||||\rPSS|1||1|202970&KRW|보험사제출서류|\rPSG|1||1|Y|16970&KRW|진찰료|\rPID|||27^^^^PI~1805314025145^^^^SS||류^수지|\rIN1|1|NHI|NHIS||||||||||||||||||||||||||||||||||5091&KRW|||||||||||||||||||\rIN2|||||||||||||||||||||||||||||MMD^^^AT&11879&KRW||||||||||||||||||||||||\rPSL|1||1|||P|AA||초진진찰료|20221113034204|||16970&KRW|1|16970&KRW|5091&KRW|||||Y|||||||1||||||||||||||||||||1|\rPSG|2||2|Y|186000&KRW|투약료|\rPSL|1||1|||P|L281||아토베리어 잇칭 크림 MD|20221113034204|||62000&KRW|1|62000&KRW|62000&KRW|||||Y|||||||2||||||||||||||||||||1|\rPSL|2||2|||P|L209||더마베이비 프로 크림 MD|20221112034204|||62000&KRW|2|124000&KRW|124000&KRW|||||Y|||||||2||||||||||||||||||||1|"
    //console.log(hl7.substring(0, 20)==data.hl7.substring(0, 20))
    
    var pmsg = data.hl7.replaceAll("\\r", "\r");
    var newpmsg = pmsg.replaceAll("\r ", "\r")
    
    //console.log(pmsg.substring(65, 85))
    //console.log(hl7.substring(65, 85))
    var message = hl7parser.create(hl7)
    // const message = hl7parser.create(""+ data.toString())
    
    //var message = hl7parser.create()
    //var hl7 = new Message(data)
    //console.log(data)
